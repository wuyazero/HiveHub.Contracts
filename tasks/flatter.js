...
subtask(
  "flatter:get-flattened-sources",
  "Returns all contracts and their dependencies flattened"
)
  .addOptionalParam("files", undefined, undefined, types.any)
  .addOptionalParam("output", undefined, undefined, types.string)
  .setAction(async ({ files, output }, { run }) => {
    const dependencyGraph = await run("flatter:get-dependency-graph", {
      files,
    });
    ...
    let licenses = flattened
      .match(/SPDX-License-Identifier:.*$/gm)
      .map((_) => _.trim());
...
subtask("flatter:get-dependency-graph")
  .addOptionalParam("files", undefined, undefined, types.any)
...
task(
  "flatter",
  "Improved flattener that produces straight-away compilable flattened contract"
)
  ...
  .setAction(async ({ files, output }, { run }) => {
    console.log(
      await run("flatter:get-flattened-sources", {
        files,
        output,
      })
    );
